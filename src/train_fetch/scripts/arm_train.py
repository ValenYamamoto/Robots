...
def get_model_actor(input_dims, output_dims):
    ...
    oldpolicy_pred = layers.Input(
        shape=(
            1,
            output_dims,
        )
    )
    advantages = layers.Input(
        shape=(
            1,
            1,
        )
    )
    rewards = layers.Input(
        shape=(
            1,
            1,
        )
    ...
    model = keras.Model(
        inputs=[inputs, oldpolicy_pred, advantages, rewards, values],
        outputs=[out],
        name="actor_model",
    )
...
def ppo_loss(oldpolicy_preds, advantages, rewards, values):
    def loss(y_true, y_pred):
        ...
        p2 = (
            K.clip(ratio, min_value=1 - CLIPPING_VALUE, max_value=1 + CLIPPING_VALUE)
            * advantages
        )
        ...
        total_loss = (
            CRITIC_DISCOUNT * critic_loss + actor_loss
        )  # - ENTROPY_BETA * K.mean( -(y_pred * K.log(y_pred+1e-10)) )
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        for it in range(PPO_STEPS):
            ...
            action = model_actor.predict(
                [current_state_tensor, DUMMY_N, DUMMY_1, DUMMY_1, DUMMY_1], batch_size=1
            )
        ...
        actor_loss = model_actor.fit(
            [
                np.reshape(np.zeros(7), (-1, NUM_JOINTS)),
                np.reshape(actions, (-1, 1, OUTPUT_DIMS)),
                np.reshape(advantages, (-1, 1, 1)),
                np.reshape(rewards, (-1, 1, 1)),
                np.reshape(values[:-1], (-1, 1, 1)),
            ],
            [(np.reshape(np.ones(7), newshape=(-1, 7)))],
            verbose=True,
            shuffle=True,
            epochs=10,  # , callbacks=[tensor_board]
        )
        print("TRAINING CRITIC")
        critic_loss = model_critic.fit(
            [np.reshape(states, (-1, NUM_JOINTS))],
...
def ppo_random_init(model_actor, model_critic, arm_controller):
    ...
    for epoch in range(1):
        ...
        actor_loss = model_actor.fit(
            ...
            [(np.reshape(np.array([1, 1, 1, 1, 1, 1, 1]), newshape=(-1, 7)))],
            verbose=True,
            shuffle=True,
            epochs=10,  # , callbacks=[tensor_board]
        )
