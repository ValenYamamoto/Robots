...
def check_valid_move(current_state, new_state, action):
    pos_tol = [
        max(0.05, 0.2 * abs(x - y).item()) for x, y in zip(action[0], current_state)
    ]
...
def reward_function(
    ...
    is_done,
    params,
    box_position=(0.75, 0, 0),
    box_size=0.5,
):
    distance_to_goal = distance_to_goal_reward_function(distance, params)
    distance_to_cube = distance_to_cube_reward_function(
        point, box_position, box_size, params["reward_constants"]["box_threshold"]
    )
    distance_moved = distance_moved_reward_function(distance, last_distance)
    return (
        params["reward_constants"]["goal_distance_constant"] * distance_to_goal
        + params["reward_constants"]["cube_distance_constant"]
        * min(distance_to_cube, 0)
...
def ppo_train_loop(
    arm_controller, model, params, joint_data, link_data, move_model, logfilename
):
    # set up loss function and optimizers
    critic_mse = torch.nn.MSELoss()
    actor_optimizer = torch.optim.Adam(
        model_actor.parameters(), lr=params["params"]["actor_lr"]
    )
    critic_optimizer = torch.optim.Adam(
        model_critic.parameters(), lr=params["params"]["critic_lr"]
    )
    ...
    for epoch in range(params["params"]["num_epochs"]):
        if epoch != 0 and epoch % 5 == 0:  # save models
            torch.save(
                ...
                f"/home/simulator/Robots/src/train_fetch/saved_models/actor{epoch}",
            )
            torch.save(
                model_critic.state_dict(),
                f"/home/simulator/Robots/src/train_fetch/saved_models/critic{epoch}",
            )
        ...
        for it in range(params["params"]["ppo_steps"]):
            ...
            current_state_tensor = torch.tensor(
                current_state.reshape((1, params["params"]["num_joints"]))
            )
            ...
            distance, done = utils.distance_from_goal(
                new_state_grip_pos, params["arm_constants"]["gripper_goal"]
            )
            ...
            if done:
                ...
                utils.reset_arm(
                    arm_controller,
                    params["arm_constants"]["initial_arm_position"],
                    move_model,
                )
                last_distance = params["arm_constants"]["initial_distance"]
        ...
        entropies = torch.mean(
            torch.reshape(
                torch.stack(entropies, dim=1), (-1, params["params"]["num_joints"])
            ),
            dim=1,
            keepdim=True,
        )
        actions = torch.reshape(
            torch.stack(actions, dim=1), (-1, params["params"]["num_joints"])
        )
        action_probs = torch.reshape(
            torch.stack(action_probs, dim=1), (-1, params["params"]["num_joints"])
        )
        old_probs = torch.reshape(
            torch.stack(old_probs, dim=1), (-1, params["params"]["num_joints"])
        )
        ...
        open_file = utils.get_logfile(logfilename)
        log_string = (
            f"LOSS epoch {epoch} actor {loss.item()} critic {critic_loss.item()} \n"
        )
        open_file.write(log_string)
        open_file.close()
...
def ppo_test(
    ...
    # reset arm
    utils.reset_arm(
        arm_controller, params["arm_constants"]["initial_arm_position"], move_model
    )
    # get current joint state
    current_state = utils.get_joint_data(joint_data[0])
