...
class ActorModel(tf.keras.Model):
    ...
    def call(
        ...
        if train:
            loss = custom_loss(
                action_probs, probs, entropies, advantages, rewards, values
            )
            self.add_loss(loss)
        else:
...
def get_model_actor(input_dims, output_dims):
    ...
    model = keras.Model(
        inputs=[inputs, action_probs, probs, entropies, advantages, rewards, values],
        outputs=[mu],
        name="actor_model",
    )
...
def ppo_use_models(model_actor, model_critic, arm_controller=None):
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        for it in range(PPO_STEPS):
            ...
            mu, sigma = prediction[:, :OUTPUT_DIMS], tf.math.abs(
                prediction[:, OUTPUT_DIMS:]
            )
            sigma = sigma + 1e-4
        ...
        actor_loss = model_actor.fit(
            tf.zeros([PPO_STEPS, NUM_JOINTS]),
            tf.zeros([PPO_STEPS, OUTPUT_DIMS]),
            verbose=True,
            shuffle=False,
            epochs=10,
            steps_per_epoch=1,
